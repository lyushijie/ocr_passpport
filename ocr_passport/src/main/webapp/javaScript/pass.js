...
function save_ocr(){
    ...
    ssr += airline_code;
    ssr += " HK1 ";
    ssr += "P"+"/"+"CHN"+"/"+ocr_id+"/"+"CHN"+"/"+ocr_birth+"/"+ocr_sex+"/"+ocr_expiry+"/"+ocr_name+"/"+"P";
    $("#ocr_format").val(ssr);
    var passport = {};
    ...
    passport.birth = ocr_birth;
    passport.expiry = ocr_expiry;
    passport.status = 2;
    if(!$.isEmptyObject(passport)){
        $.post("pass/renewal",passport);
    }
}
